    ...
    <div className="container-store flex min-h-[70vh] items-center justify-center py-12">
      <div className="w-full max-w-md">
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="mt-8 space-y-4">
          ...
          <div>
            <label className="text-sm font-medium" htmlFor="username">Email / Username</label>
            <Input id="username" className="mt-1" {...register('username')} autoComplete="username" />
            {errors.username && <p className="mt-1 text-xs text-destructive">{errors.username.message}</p>}
          </div>
          <div>
            <label className="text-sm font-medium" htmlFor="password">Password</label>
            <Input id="password" type="password" className="mt-1" {...register('password')} autoComplete="current-password" />
            {errors.password && <p className="mt-1 text-xs text-destructive">{errors.password.message}</p>}
          </div>
          <div className="flex justify-end">
            <Link href={ROUTES.forgotPassword} className="text-sm text-muted-foreground hover:text-foreground">
              Forgot password?
            </Link>
          </div>
